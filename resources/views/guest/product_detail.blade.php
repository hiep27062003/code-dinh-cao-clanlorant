                        ...
                        <div class="details-pro">
                            ...
                            <form enctype="multipart/form-data" data-cart-form="" id="add-to-cart-form"
                                ...
                                <script>
                                    ...
                                    jQuery(document).ready(function($) {
                                        ...
                                        const haveVariant = {{ $product->have_variant ?? 0 }};
                                        ...
                                        if (haveVariant == 1) {
                                            ...
                                            // Sản phẩm không có variant - gọi API với null
                                            if (productId) {
                                                fetchProductPrice(productId, null, null);
                                            }
                                        }
									...
									function fetchProductPrice(productId, storage, color) {
                                        console.log(`Fetching price for: Product=${productId}, Storage=${storage || 'null'}, Color=${color || 'null'}`);
                                        
                                        // Chuẩn bị data - chỉ gửi storage/color nếu có
                                        const requestData = {
                                            product_id: productId
                                        };
                                        
                                        // Chỉ thêm storage và color nếu không null (sản phẩm có variant)
                                    ...
                                    <form id="productForm" method="POST">
                                        ...
                                        <div class="group-button">
                                            ...
                                            <button type="button" style="width: 100%" class="btn-buyNow"
                                                {{-- Update onclick to use the new route and action type --}}
                            ...
                            <script>
                                ...
                                function updateValues() {
                                    ...
                                    if (haveVariant == 1) {
                                        // Sản phẩm có variant - lấy giá trị từ form
                                        selectedStorage = jQuery('#selected-storage').val() || '';
                                        selectedColor = jQuery('.color-selector:checked').val() || '';
                                    } else {
                                        // Sản phẩm không có variant - để trống (sẽ gửi null)
                                        selectedStorage = '';
                                        selectedColor = '';
                                    }
                                    
									var selectedQuantity = jQuery('#qtym').val() || '1';
                                    ...
                                    console.log("Updated hidden fields:", {
                                        ...
                                        storage: selectedStorage || 'null (no variant)',
                                        color: selectedColor || 'null (no variant)',
                                ...
                                function submitForm(action, actionType = 'add_to_cart') {
                                    ...
                                    // Chỉ yêu cầu chọn màu/dung lượng nếu sản phẩm có variant
                                    if (haveVariant == 1 && (!selectedColor || !selectedStorage)) {
                                    ...
                                    // Chuẩn bị data - gửi null nếu không có variant
                                    const requestData = {
                                        _token: csrfToken,
                                        product_id: productId,
                                        quantity: selectedQuantity
                                    ...
                                    // Chỉ thêm color và storage nếu sản phẩm có variant
                                    if (haveVariant == 1) {
                                        requestData.color = selectedColor || '';
                                        requestData.storage = selectedStorage || '';
                                    } else {
                                        // Sản phẩm không có variant - gửi null
                                        requestData.color = null;
                                        requestData.storage = null;
                                    }
...
<script>
    ...
    // Dòng này lấy trạng thái từ Laravel và in vào biến JS
    ...
    // 2. HÀM CẬP NHẬT GIÁ TRỊ INPUT ẨN
    function updateValues() {
        // Kiểm tra xem sản phẩm có variant không
        const haveVariant = {{ $product->have_variant ?? 0 }};
        ...
        if (haveVariant == 1) {
            // Sản phẩm có variant - lấy giá trị từ form
            selectedStorage = $('#selected-storage').val() || '';
            selectedColor = $('.color-selector:checked').val() || '';
        } else {
            // Sản phẩm không có variant - để trống (sẽ gửi null)
            selectedStorage = '';
            selectedColor = '';
        ...
        var selectedQuantity = $('#qtym').val() || '1';
    ...
    // 3. HÀM GỬI FORM (CÓ CHUYỂN HƯỚNG ĐĂNG NHẬP)
    function submitForm(action, actionType = 'add_to_cart') {
        
        // ==> KIỂM TRA: NẾU CHƯA ĐĂNG NHẬP THÌ CHUYỂN TRANG LOGIN NGAY <==
        if (!isLoggedIn) {
        ...
        const selectedStorage = $('#selectedStorage').val();
        ...
        // Chỉ yêu cầu chọn màu/dung lượng nếu sản phẩm có variant
        if (haveVariant == 1 && (!selectedColor || !selectedStorage)) {
            ErrorNoti('Vui lòng chọn Màu sắc và Dung lượng trước khi mua.');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    if (actionType === 'buy_now_direct' && response.redirect_url) {
                        window.location.href = response.redirect_url;
                    } else {
                        SuccessNoti(response.message || 'Thành công!');
                    }
                } else {
                    if(response.exists) {
                         SuccessNoti(response.message);
                    } else {
                         ErrorNoti(response.message || 'Có lỗi xảy ra.');
                    }
                }
            },
            error: function(xhr) {
                ...
                // Phòng hờ session hết hạn giữa chừng
                if (xhr.status === 401) {
                    window.location.href = loginUrl;
                } else {
    ...
    // 4. KHỞI TẠO LIGHTGALLERY (AN TOÀN)
    $(document).ready(function() {
        try {
            if ($.fn.lightGallery) {
                ...
                console.warn("Thư viện lightGallery chưa được load.");
            }
        } catch (e) {
            console.log("Lỗi khởi tạo LightGallery:", e);
        }
    });
</script>
