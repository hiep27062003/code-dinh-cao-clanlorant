    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6 border-b border-gray-700 pb-3">
            <div class="flex items-center gap-3">
                <i class="bi bi-plus-circle text-3xl text-green-400"></i>
                <h1 class="text-3xl font-bold text-gray-100">Tạo Tin Tức Mới</h1>
            </div>
        ...
        <form action="{{ route('news.store') }}" method="POST" enctype="multipart/form-data" class="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea name="content" id="tinymce-editor" class="w-full @error('content') border-red-500 @enderror" required>{{ old('content') }}</textarea>
                @error('content')
                    <div class="text-red-400 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="is_active" id="is_active" 
                    ...
                    <option value="0" {{ old('is_active', 0) == 0 ? 'selected' : '' }}>Ẩn</option>
                </select>
            </div>
