    ...
    public function vnpayCallback(Request $request)
{
    ...
    if (!$order) {
        ...
        // Redirect về trang guest, KHÔNG CẦN AUTH
    ...
    if ($secureHash === $vnp_SecureHash) {
        if ($responseCode == '00') {
            ...
            \Log::info('Payment successful:', ['order_code' => $orderCode]);
            
            // Redirect về guest index với thông báo
            return redirect()->route('guest.index')
                ->with('success', 'Thanh toán thành công! Mã đơn hàng: ' . $order->order_code);
        } else {
