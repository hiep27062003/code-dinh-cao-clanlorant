...
class CustomerController extends Controller
{
    ...
    public function getPrice(Request $request)
    {
        try {
            ...
            // Lấy thông tin sản phẩm để kiểm tra have_variant
            ...
            // Kiểm tra sản phẩm và danh mục có active không
            if ($product->status !== 'active' || $product->category->status !== 'active') {
                return response()->json([
                    'success' => false,
            ...
            // Nếu sản phẩm không có variant, tìm variant với color/storage = null
            if ($product->have_variant == 0) {
                ...
                // Sản phẩm có variant - cần storage và color
                if (!$storage || !$color) {
                    return response()->json([
                        'success' => false, 
                        'message' => 'Vui lòng chọn màu sắc và dung lượng'
                    ], 400);
                }
    ...
    public function postCart(Request $request): JsonResponse
    {
        ...
        // Nếu sản phẩm không có variant (have_variant = 0), tìm variant với color/storage = null
        if ($product->have_variant == 0) {
    ...
    public function postCartDT(Request $request): JsonResponse
    {
        ...
        'product_id' => 'required|integer|exists:products,id',
             'color' => 'nullable|string', // Cho phép null cho sản phẩm không có variant
             'storage' => 'nullable|string', // Cho phép null cho sản phẩm không có variant
             'quantity' => 'required|integer|min:1|max:5',
         ]);
        ...
        // Kiểm tra sản phẩm và danh mục có active không
        if ($product->status !== 'active' || $product->category->status !== 'active') {
            return response()->json(['success' => false, 'message' => 'Rất tiếc, có thể sản phẩm hết hàng hoặc đã ngừng kinh doanh.'], 404);
        }
        ...
        if ($product->have_variant == 0) {
            ...
            // Sản phẩm có variant, cần color và storage
            if (empty($data['color']) || empty($data['storage'])) {
                return response()->json(['success' => false, 'message' => 'Vui lòng chọn màu sắc và dung lượng sản phẩm.'], 422);
            }
            $product_variant = ProductVariant::where('product_id', $data['product_id'])
    ...
    public function proceedDirectlyToCheckout(Request $request)
    {
        ...
        // hơp lệ, lưu thông tin vào session
